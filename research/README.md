...
- **E7 — landed contributions:** two-turn test (user says "yes, add it" — does the agent submit a well-formed stub?) + one scenario with a clean *public* gap unrelated to the user, isolating the "contribute freely" branch. Measure attempted+offered, not just landed: agent platforms gate outbound POSTs (observed: a willing agent's note was blocked by its own harness permissions).
