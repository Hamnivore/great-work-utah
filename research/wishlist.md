...
- **Regional overlays (shipped, coverage expanding, 2026-07-14).** The privacy/provenance schema, correction/removal path, generated `/locations.geojson`, `/api/locations` proximity endpoint, and human `/map` are live. Five cold probes discovered and used the endpoint; production desktop/mobile verification passed. Omit residences, private workplaces, and individuals; use approximate anchors only for safe public areas/sites where exactness is unnecessary or unsupported. Remaining corpus work: expand beyond the initial 8/393 region-attributed pages without weakening the evidence or privacy bar.
