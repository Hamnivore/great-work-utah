...
- **Regional overlays (shipped, coverage expanding, 2026-07-14).** The privacy/provenance schema, correction/removal path, generated `/locations.geojson`, `/api/locations` proximity endpoint, and human `/map` are live. Verified public sites use exact points; people and entities without a safe public site use only coarse region anchors, never residences or private addresses. Five cold probes discovered and used the endpoint; production desktop/mobile verification passed. Remaining corpus work: improve regional coverage and replace coarse anchors with exact points only when a clearly public site is sourced.
