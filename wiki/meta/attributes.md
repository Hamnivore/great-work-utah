...
| Builder-tier | venture, person, helper, resource, work | yes | S · A · B · C · D · F · unranked — builder character visible in the work under a maximum-faith reading; independent of impact and never a claim about private moral worth. Rubric in [`builder-tiers.md`](builder-tiers.md) | builder tier list, hidden-gem gap view |
...
| Activity | venture, person, helper, resource, work | rollout in progress | active · dormant · concluded · unknown — whether the subject is *still being done*, the only attribute that grades the subject rather than the page or its evidence. What the mark means: [`activity.md`](activity.md). Assigner rubric: [`research/activity/rubric.md`](../../research/activity/rubric.md) | `(active)` on the tier list and type indexes, currency badge on prerendered pages |
