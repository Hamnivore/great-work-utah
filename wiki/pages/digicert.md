# DigiCert

**Type:** venture
**Status:** Useful
**Confidence:** Medium
**Tier:** B
**Builder-tier:** B
**Activity-signal:** 2026-05-12 · https://finance.yahoo.com/sectors/technology/articles/digicert-receives-frost-sullivan-2026-130000775.html
**Activity-checked:** 2026-08-14
**Activity:** active
**Focus:** public-key infrastructure, certificate lifecycle management, digital trust, DNS, post-quantum security
**Roles:** software-engineering
**Stage:** Established global cybersecurity company; private-equity backed
**Primary Location:** Lehi, UT
**Utah Location:** Lehi, UT
**Region:** Lehi
**Map Location:** 2801 North Thanksgiving Way, Suite 500, Lehi, UT 84048
**Coordinates:** 40.423474, -111.887431
**Location Precision:** exact
**Location Source:** https://privacy.digicert.com/policies/en/?name=job-candidate-privacy-notice
**Careers:** https://www.digicert.com/careers
**Website:** https://www.digicert.com/
**Domain:** computing
**Ownership:** PE-owned; backed by Clearlake Capital, Crosspoint Capital Partners, and TA Associates
**Updated:** 2026-08-11
**Needs-reviewed:** 2026-08-11

## Summary

DigiCert is a Lehi-based cybersecurity company whose DigiCert ONE platform manages public-key infrastructure, digital certificates, DNS, and certificate lifecycles. Digital certificates are quiet infrastructure: when they work, software and people can authenticate systems and encrypt traffic; when issuance, renewal, or trust chains fail, large services can become insecure or unavailable.

## Impact

Digital certificates are quiet infrastructure: when they work, software and people can authenticate systems and encrypt traffic; when issuance, renewal, or trust chains fail, large services can become insecure or unavailable. DigiCert's impact comes from making that infrastructure manageable at enterprise scale and helping standards and customers migrate toward post-quantum cryptography. The bounds are substantial: market position and customer counts do not by themselves establish security quality, and concentration in a few certificate authorities creates systemic as well as operational leverage.

## Utah Context

DigiCert's principal offices are in Lehi, where it anchors high-skill cybersecurity and infrastructure work. Its official materials also identify Lehi as one of four global office hubs and document local STEM work with Thanksgiving Point.

## What They Need Now

DigiCert's current careers page is the durable application path for its global engineering organization. Fresh Lehi recruiting specifically sought senior and principal site-reliability engineers working across AWS, Kubernetes/EKS, Terraform, automation, platform reliability, and incident-resilient infrastructure. The durable Utah fit is therefore software, cloud-platform, SRE, security engineering, PKI, and systems architecture rather than a single expiring opening; candidates should filter the official board for Lehi or Utah.

## Open Questions

- How many roles and employees remain anchored in Lehi after acquisitions and private-equity ownership changes?
- What independent measures best capture certificate-authority reliability and readiness for post-quantum migration?
- The exact map coordinate is geocoded from the official candidate-privacy address rather than a canonical site coordinate published by DigiCert.

## Evidence

- [Source: DigiCert Official Website — About](digicert-official-website.md) · https://greatutah.work/pages/digicert-official-website.md · https://www.digicert.com/about
- [Source: DigiCert to Acquire Vercara](digicert-vercara-acquisition.md) · https://greatutah.work/pages/digicert-vercara-acquisition.md · https://www.ta.com/news/digicert-to-acquire-vercara-strengthening-its-position-as-a-leader-in-digital-trust/
- DigiCert careers · https://www.digicert.com/careers — durable hiring board and employer information.
- DigiCert candidate privacy notice · https://privacy.digicert.com/policies/en/?name=job-candidate-privacy-notice — principal-office address and applicant-data controller.

## Maintainer Notes

- Replace the geocoded map coordinate if DigiCert publishes a canonical site coordinate.
