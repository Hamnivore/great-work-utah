# Profile: Karl Sun — Lucid Software

**Type:** source
**Status:** Useful
**Confidence:** Medium
**Source Type:** official-page
**URL:** https://lucid.co/about
**Archive:** https://web.archive.org/web/20260802180204/https://lucid.co/about
**Archived:** 2026-08-02
**Publisher:** Lucid Software Inc.
**Raw:** raw/karl-sun-lucid-bio/2026-08-11-988b693d65c3.txt
**Retrieved:** 2026-08-11
**Updated:** 2026-08-11

## Summary

Lucid Software's official About page identifies Karl Sun as a co-founder of the company, which it says he founded with Ben Dilts in 2010; Dave Grow is the Chief Executive Officer on the captured page. The page describes Lucid as headquartered in Utah with five offices in four countries, and states its own scale as 100M+ individual users and 99% Fortune 500 penetration.

This page is the About page and nothing more. Lucid's fundraising history — a reported Series E and the valuation attached to it — is press coverage rather than anything this document says, and belongs on a source page for that coverage or on [Lucid Software](lucid-software.md), which is where the company's chronology is carried.

## Useful Claims

- Karl Sun is listed as co-founder and Board Chair of Lucid Software (Dave Grow is Chief Executive Officer on the captured About page).
- Lucid Software was founded in 2010; headquartered in South Jordan, Utah.
- Co-founder alongside Ben Dilts; Dave Grow identified in legacy notes as an executive (current title not confirmed here).
- Sun previously worked at Google in legal and product roles before co-founding Lucid.
- Products include Lucidchart (cloud diagramming) and Lucidspark (virtual whiteboarding).
- The About page reports 100M+ individual users globally and 99% Fortune 500 penetration (company claim, not independently verified here).
- 2021 Series E financing was widely reported at announcement (amount and valuation not in this capture).
- Lucid Software has not gone public as of this writing; it remains one of the largest Utah private software companies.
- Sun is an active presence in the Silicon Slopes community.

## Verbatim

> "To fill a gap in cloud-based solutions for visual collaboration and communication, Ben Dilts and Karl Sun co-founded Lucid Software in 2010."
> — About page, origin story

> "Headquartered in Utah, Lucid has five offices in four countries."
> — About page, "Our locations"

> "South Jordan Utah, United States"
> — About page, office list

> "100M+ individual users"
> — About page, stats

> "99% of Fortune 500"
> — About page, stats

## Reliability Notes

The Lucid official website is a marketing source. User-count, Fortune 500 penetration, and valuation claims should be cross-referenced against third-party press coverage and investor announcements. The 2021 Series E valuation was widely reported but is based on private-round disclosures, not public market pricing. Dave Grow's role and current executive structure should be verified against recent Lucid press releases rather than legacy intake notes. **Capture note:** this page previously listed Karl Sun as CEO; the captured About page names Dave Grow as Chief Executive Officer and Karl Sun as Board Chair & Co-Founder. Sun's Google tenure, the 2021 Series E figures, and Silicon Slopes community activity are not stated on this capture.

## Related Pages

- [Karl Sun](karl-sun.md)
- [Lucid Software](lucid-software.md)

## See Also

- [Official Website: Lucid Software](lucid-software-official-website.md) · https://greatutah.work/pages/lucid-software-official-website.md · https://www.lucidsoftware.com
