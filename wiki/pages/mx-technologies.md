# MX Technologies

**Type:** venture
**Status:** Draft
**Confidence:** Medium
**Focus:** open banking, financial data, account aggregation, fintech API platform
**Stage:** Private; founded 2010 (as MoneyDesktop); Lehi HQ
**Primary Location:** Lehi, UT
**Utah Location:** Lehi, UT
**Region:** Lehi
**Careers:** https://www.mx.com/careers/
**Updated:** 2026-06-21
**Needs-reviewed:** 2026-06-21
**Pull:** *One of Silicon Slopes' leading private fintech companies — open banking infrastructure for banks, credit unions, and fintech developers.*

## Summary

MX Technologies is a financial technology company that provides an open banking platform to banks, credit unions, and fintech developers. The platform enables secure access to financial data, data enrichment (categorization, cleaning, normalization), account aggregation, and payment initiation through APIs. MX helps financial institutions offer personalized digital experiences and comply with consumer data-sharing regulations such as the CFPB's Section 1033 rulemaking. Founded in 2010 as MoneyDesktop, the company rebranded to MX in 2015 and is a founding member of the Financial Data Exchange (FDX), a nonprofit focused on standardizing consumer financial data access.

## Impact

MX sits at the infrastructure layer of the open banking movement — enabling the data sharing that powers fintech apps, financial planning tools, and bank digital experiences. Its FDX membership and regulatory engagement position it as a policy-active player in how consumer financial data rights get implemented.

## What They Need Now

Software engineers, data engineers, product managers, financial-industry relationship managers, and regulatory affairs specialists. The company's Lehi location makes it a core employer in Utah's fintech cluster.

## Utah Context

MX is headquartered in Lehi, a hub in the Silicon Slopes tech ecosystem. The company is one of Utah's most influential private fintech companies and a frequent reference point for the state's financial technology reputation alongside companies like [Podium](podium.md), [Qualtrics](qualtrics.md), and [BambooHR](bamboohr.md).

## Evidence

- [Official Website](https://www.mx.com) · https://www.mx.com

## Open Questions

- Current valuation and funding status — has MX pursued a path to public markets?
- How is MX positioning relative to Plaid, Finicity (Mastercard), and other aggregators in the open-banking competitive set?
- Ryan Caldwell's current strategic priorities?
