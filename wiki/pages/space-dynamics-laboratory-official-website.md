# Official Website: Space Dynamics Laboratory

**Type:** source
**Status:** Useful
**Confidence:** Medium
**Source Type:** official-page
**URL:** https://www.sdl.usu.edu
**Archive:** https://web.archive.org/web/20260601165713/https://www.sdl.usu.edu/
**Archived:** 2026-06-01
**Publisher:** Space Dynamics Laboratory / Utah State University
**Raw:** raw/space-dynamics-laboratory-official-website/2026-08-11-061b4c51b44e.txt
**Retrieved:** 2026-08-12
**Updated:** 2026-08-11

## Summary

Space Dynamics Laboratory's official website homepage describes SDL as a University Affiliated Research Center and lists public capability areas, news, and contact information. This capture is a thin home page; detailed history, ownership, and workforce claims require About/History page captures.

## Useful Claims

- The homepage describes SDL as "A University Affiliated Research Center (UARC) that delivers cutting-edge mission engineering and technologies that advance critical national priorities across all domains."
- As a UARC, the capture states SDL maintains essential engineering, research, and development capabilities for the DoW.
- Public capability areas listed on the homepage include Satellites, Sensors & Instruments, Ground & Data, and Autonomous Systems.
- Contact address on the capture: 416 East Innovation Avenue, North Logan, UT 84341.
- This capture does not state SDL's founding history, payload flight counts, nonprofit/USU ownership, or dual Space Force and Missile Defense Agency UARC designations.

## Verbatim

> "A University Affiliated Research Center (UARC) that delivers cutting-edge mission engineering and technologies that advance critical national priorities across all domains."
> — Home page, organization description

> "As a UARC, we maintain essential engineering, research, and development capabilities for the DoW."
> — Home page, event description

## Reliability Notes

This capture is a thin home page. It does not substantiate the nonprofit/USU ownership, dual-UARC distinction, 1,300+ employees, 430+ payloads, detailed history, student workforce, or named science-program claims; those require captures of the About, History, and program pages.

This is an official institutional source. SDL's self-description of structure, UARC designations, employee count, and competency areas is reliable. Much of SDL's most consequential defense work is classified or customer-sensitive and therefore not described on the public website; impact claims from this source alone should be treated as incomplete. The dual Space Force/MDA UARC designation and mission flight count (430+ payloads) can be cross-checked against government contracting databases and Wikipedia.

## Related Pages

- [Space Dynamics Laboratory](space-dynamics-laboratory.md)

## Maintainer Notes

- Capture SDL's About page before relying on the previously recorded employee count, multi-state office footprint, and North Logan headquarters.
