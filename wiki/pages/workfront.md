# Workfront

**Type:** venture
**Status:** Draft
**Confidence:** Medium
**Tier:** C
**Activity-signal:** 2026-08-13 · https://experienceleague.adobe.com/en/docs/workfront/using/product-announcements/product-releases/release-26-q4/26-q4-release-overview
**Activity-checked:** 2026-08-14
**Activity:** active
**Focus:** enterprise work management, marketing operations, project management, Adobe Experience Cloud
**Roles:** program-project-management
**Domain:** computing
**Primary Location:** Lehi, UT
**Utah Location:** Lehi, UT
**Region:** Lehi
**Stage:** Acquired by Adobe in 2020; operates as Adobe Workfront
**Ownership:** public
**Website:** https://business.adobe.com/products/workfront/main.html
**Careers:** https://careers.adobe.com/us/en/
**Updated:** 2026-07-14
**Needs-reviewed:** 2026-08-11
**Pull:** *A Utah-founded enterprise work-management company that became a $1.5 billion Adobe acquisition and remains an active Adobe product.*

## Summary

Workfront is an enterprise work-management platform founded in Utah in 2001 under the AtTask name. The product continues as Adobe Workfront for planning, coordinating, and tracking enterprise work.
## Impact

At acquisition scale, Workfront reached roughly one million users across more than 3,000 customers, so its breadth extended well beyond Utah. The platform goes deeper than task tracking: it coordinates planning, resources, approvals, and execution across enterprise teams. Its persistence is visible in Adobe's continued investment in Workfront as a named product integrated with Adobe's marketing and creative tools.

The counterfactual is less certain. Enterprise work-management software would exist without Workfront, but a Utah-founded company built one of the category's large platforms and carried it into a global software portfolio through a ten-figure acquisition. How much independent product direction and Utah employment persisted after the acquisition is not publicly broken out.

## What They Need Now

Workfront is an Adobe product, so applicants should use Adobe's careers site and filter for Utah or remote roles. A recently filled Workfront-specific Lehi listing was for product operations and documentation management, supporting `program-project-management` as the narrowest durable role family. It is evidence of the function, not a current opening; do not infer a Workfront assignment from an Adobe job's location alone.

## Utah Context

AtTask was founded in 2001 and later headquartered in Lehi before adopting the Workfront name in 2015. Its acquisition gave Utah a second major enterprise-software connection to Adobe after Adobe's 2009 acquisition of [Omniture](omniture-web-analytics.md). The available public sources do not isolate current Workfront-specific headcount in Utah.

## Evidence

- [AtTask Changes its Name to Workfront Inc.](https://www.prnewswire.com/news-releases/attask-changes-its-name-to-workfront-inc-300023764.html) — company-issued 2015 rebrand announcement; states the 2001 founding date and contemporary scale.
- [Adobe Completes Acquisition of Workfront](https://www.adobe.com/cc-shared/assets/investor-relations/pdfs/120720adobecompletesacquisitionofworkfront.pdf) — acquisition price, closing date, customer/user scale, and intended Adobe integration.
- [Adobe Workfront product page](https://business.adobe.com/products/workfront/main.html) — current product positioning and integrations.
- [Adobe](adobe.md) · https://greatutah.work/pages/adobe.md

## Open Questions

- How many Adobe employees in Utah work primarily on Workfront today?
- Which Workfront engineering, product, and customer-facing functions remain based in Lehi?
- What product and employment outcomes would have differed if Workfront had remained independent?
