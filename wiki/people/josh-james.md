# Josh James

**Status:** Draft
**Confidence:** Medium
**Focus:** enterprise SaaS, web analytics, business intelligence, Utah tech ecosystem
**Location:** Utah Valley, UT
**Updated:** 2026-06-19
**Relates:** cites [Profile: Josh James — Omniture and Domo](../sources/josh-james-domo-bio.md)

## Summary

Josh James is a Utah entrepreneur best known for co-founding Omniture and Domo — two enterprise software companies he built back-to-back in Utah Valley. He co-founded Omniture in 1996 with John Pestana, led the company through its NASDAQ IPO in 2006, and sold it to Adobe for $1.8 billion in 2009 — the largest Utah tech exit of its era at the time. He left Adobe in 2010 and founded Domo, a business intelligence and data integration platform that went public on NASDAQ in 2018 (ticker: DOMO).

## Impact

James's two companies form the clearest through-line of Silicon Slopes as an enterprise software destination. The Omniture acquisition trained a generation of Utah SaaS operators and proved that a Utah-based company could compete at enterprise scale globally. His angel investment in companies like [Xandem](../ventures/xandem.md) signals continued ecosystem engagement. For later Utah founders, the James playbook — build in Utah Valley, serve enterprise buyers globally, exit at scale — is one of the best-documented templates available.

## What They Offer

Operational depth in building enterprise data products and analytics companies. His primary ecosystem contribution is the Omniture exit and the downstream operator community it created, alongside Domo as an ongoing existence proof of a Utah-built public cloud software company.

## What They Are Looking For

Not known from public sources. James appears primarily focused on leading Domo through its next strategic phase. His Utah civic engagement suggests continued openness to ecosystem-level conversations.

## Proof of Work

- Co-founded Omniture (1996); NASDAQ IPO (2006); Adobe acquisition for $1.8B (2009)
- Founded Domo (2010); NASDAQ IPO (2018, ticker DOMO)
- Angel investor in [Xandem](../ventures/xandem.md) and reportedly other Utah early-stage companies

## Good Fits

Utah founders building B2B data and analytics software at enterprise scale; operators looking to understand Silicon Slopes talent development; researchers studying how a Utah tech exit compounds into a broader ecosystem.

## Public Connection Path

Domo investor relations and public speaking venues. Not known to have an open intake path.

## Evidence

- [Profile: Josh James — Omniture and Domo](../sources/josh-james-domo-bio.md)
- [Omniture Web Analytics](../work/omniture-web-analytics.md)
- [Domo](../ventures/domo.md)

## See Also

- [Omniture Web Analytics](../work/omniture-web-analytics.md)
- [Domo](../ventures/domo.md)
- [Qualtrics](../ventures/qualtrics.md) — Utah's other defining enterprise SaaS exit

## Open Questions

- What is the current strategic direction of Domo and James's day-to-day role?
- What is James's public availability for ecosystem-level conversations?
